...
#include "Table.h"
#define PSEUDO 0 //Place pseudo dans la base de donnée 
#define PASSWORD 1 //Place mot de passe dans la base de donnée 
#define MONEY 2 //PLace de l'argent dans la base de donnée
#define EMAIL 3 //Place de l'adresse mail dans la base de donnée 
#define DOB 4 //Place de la date de naissance dans la base de donnée 
#define GENDER 5 //Place du genre dans la base de donnée
#define LNAME 6 //Place du nom dans la base de donnée 
#define FNAME 7 //Place du prenom dans la base de donnée 
#define ADRESS 8 //Place de l'adresse dans la base de donnée 
#define CITY 9 //Place de la ville dans la base de donnée 
...
void Table::playerReturnTable(int place){
  for(int i=0;i<NB_CHAISE;i++){
    if(joueurs[i]!=NULL && !actif[i] && spectacteurs[place]->getDesc()==joueurs[i]->getDesc()){
      actif[i]=true;
  ...
  while(strlen(recu)!=0){
	...
	if(recu[0]=='A' && recu[1]=='b' && recu[2]=='s' && recu[3]=='e' && recu[4]=='n'){
	  ...
	  if(recu[0]=='Q' && recu[1]=='u' && recu[2]=='i' && recu[3]=='t' && recu[4]=='T'){
	    ...
	    if(recu[0]=='R' && recu[1]=='e' && recu[2]=='t' && recu[3]=='o' && recu[4]=='u'){
	      strcpy(recu,recu+6);
	      playerReturnTable(placeSpec);
	    }
	    else{
